...
manuDownload=$(sort -u "$DATADIR/logs/$GENNAME"-logs.txt | grep -c "Could not download:" )
noDownload=$(sort -u "$DATADIR/logs/$GENNAME"-logs.txt | grep -c "No existing links:" )

echo "[$FINE] There are ${YELLOW}$manuDownload${NORMAL} songs that were not able to be ripped by this script."
echo "[$INFO] You may want to try downloading these songs manually"
echo ""
sort -u "$DATADIR/logs/$GENNAME"-logs.txt | grep "Could not download:"

echo ""
echo "[$SHIT] There are ${YELLOW}$noDownload${NORMAL} [ unpublished / not public ] music."
echo ""
sort -u "$DATADIR/logs/$GENNAME"-logs.txt | grep "No existing links:"
